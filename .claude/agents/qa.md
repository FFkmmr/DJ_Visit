...
Используй фреймворк из `docs/02-tech-stack.md` (pytest / vitest / jest / playwright / etc.).

#### Обязательные паттерны
- **Изолированные тесты**: каждый тест поднимает чистое состояние.
- **Реальная БД** (через testcontainers / fixtures), **не mock**, для integration.
- **Моки только для внешних сервисов** (IMAP/SMTP, third-party API).
- **Структурированные имена**: `test_<feature>_<scenario>_<expected>` (например, `test_add_mailbox_with_invalid_credentials_returns_400`).
...
- ❌ `time.sleep` в тестах — используй deterministic events / fakes.
- ❌ Игнорирование падающих тестов через `@skip` / `@xfail` без TD-NNN cross-ref.
- ❌ Тесты, которые проходят только локально (флапы из-за времени, сети).

### Шаг 4: Запуск

Запусти тесты в стеке проекта. Типичные команды:
- Python: `pytest --cov=src --cov-report=term-missing`
- Node: `npm test -- --coverage` или `vitest run --coverage`
- Go: `go test ./... -cover`
...
- ❌ НЕ используй `@skip` / `@xfail` без TD-NNN cross-ref.
