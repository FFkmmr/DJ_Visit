...
После написания кода ОБЯЗАТЕЛЬНО запусти инструменты, указанные в `docs/02-tech-stack.md` / `docs/conventions/code-style.md`. Типичные команды:

- Python: `ruff format`, `ruff check --fix`, `mypy`
- Node/TS: `prettier --write`, `eslint --fix`, `tsc --noEmit`
- Go: `gofmt`, `go vet`, `staticcheck`

Все инструменты должны пройти без ошибок. Если падают — исправь.
...
### Безопасность
- [ ] Auth middleware применён ко всем endpoints (кроме явно публичных)
- [ ] Authz проверки соответствуют RBAC документации
- [ ] Секреты — только через config / env / secret manager
...
- [ ] Нет `print()`, нет sync sleep в async, нет магических чисел
- [ ] Конкретные exception types

### Качество
- [ ] Idempotency для polling / фоновых задач
- [ ] N+1 нет
- [ ] Метрики / структурированные логи добавлены
